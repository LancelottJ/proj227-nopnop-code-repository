                                ...
                                int *container_ready_fd, libcrun_error_t *err)
{
  ...
  cgroup_manager = CGROUP_MANAGER_DISABLED; // 关闭cgroup
  // printf (" 是否打开cgroup:%d\n", cgroup_manager != CGROUP_MANAGER_DISABLED);
  ...
  // 修改cpu.shares
  def->linux->resources->cpu->shares = def->linux->resources->cpu->period;
  // printf ("  cgroup-cpu.shares = %d\n", def->linux->resources->cpu->shares);
  ...
  // 消息队列
  key_t key = ftok ("/home/femu/proj227-nopnop-code-repository/isolation/cpu/egroup/.msgq", 0);
  int msgid = msgget (key, 0644 | IPC_CREAT);

  struct msg_buf buf;
  buf.mtype = 1;
  buf.mdata.egid = 0;
  buf.mdata.pid = cg.pid;
  buf.mdata.share = cg.resources->cpu->period;

  msgsnd (msgid, &buf, sizeof (buf) - sizeof (long), 0);
  printf ("  msgsnd:egid:%d,pid:%d,shares: %ld\n", 0, cg.pid, cg.resources->cpu->period);
  // printf ("  cgroup:egid:%d,pid:%d,shares: %ld\n", 0, cg.pid, 0);
  //  消息队列
